...
if (n_waves >= length(results.list)) {
  ...
  while (i <= n_waves) {
    ...
    tr <- do.call('rbind', results.list)
    gr <- tr[which(apply(tr, 1, in.bounds, target.bounds) == length(target.bounds)),]
    if ((all(uncertainty.products < 1.1) && length(ems) == length(target.bounds) && max_imp <= 3 && nrow(gr) > 0) || sum(apply(results.list[[length(results.list)]], 1, in.bounds, target.bounds) == length(target.bounds))/nrow(results.list[[length(results.list)]]) > mine_trigger) {
